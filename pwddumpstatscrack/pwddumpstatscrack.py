...
# the users file dumped from database to analise and cracking
usersPathFile = 'company_users.csv'
# file name to store the repeated passwords found
fileWithRepeatedPass = 'file_with_repeated_passwords.txt'
# file name to store unique the passwords found
fileWithUniquePass = 'file_with_unique_passwords.txt'
# path to the passwords dictionary
dictionaryList = '/usr/share/wordlists/john.lst'
# file name to store the cracked repeated passwords
outputFileWithRepeatedCrackedPass = 'cracked_repeated_passwords.txt'
# file name to store the unique the cracked passwords
outputFileWithUniqueCrackedPass = 'cracked_all_passwords.txt'
# run hashcat -h and find the proper hash of your passwords, this example is "bcrypt $2*$, Blowfish (Unix)"
hashcatHashMode = 3200

# Users arrays
finalUniqueUsersRecords = []  # Users in file without blank/empty password
# Users with the same password -> [{password, [usernames]}]
userRepeatedPass = []
...
def createFilesForCracking():
    ...
    with open(fileWithRepeatedPass, 'w') as newFileRepeated:
        ...
        print('File with %i repeated passwords has been successfully created --> "%s".' %
              (len(userRepeatedPass), fileWithRepeatedPass))

    with open(fileWithUniquePass, 'w') as newFileUnique:
        for record in finalUniqueUsersRecords:
            newFileUnique.write("%s\n" % record[0])
        print('File with %i unique passwords has been successfully created --> "%s".' %
              (len(finalUniqueUsersRecords), fileWithUniquePass))
...
def printFinalStats():
    ...
    if os.path.exists(outputFileWithRepeatedCrackedPass):
        with open(outputFileWithRepeatedCrackedPass, 'r') as file:
            ...
            print('--> %i/%i repeated passwords were cracked.' %
                  len(userRepeatedPass))
            if passwordsCracked > 0:
                print('Check the results in "%s".' %
                      (outputFileWithRepeatedCrackedPass))
    else:
        print('--> 0/%i repeated passwords were cracked.' %
              len(userRepeatedPass))

    if os.path.exists(outputFileWithUniqueCrackedPass):
        with open(outputFileWithUniqueCrackedPass, 'r') as file:
            passwordsCracked = len(file.readlines())
            print('--> %i/%i unique passwords were cracked.' %
                  len(finalUniqueUsersRecords))
            if passwordsCracked > 0:
                print('Check the results in "%s".' %
                      (outputFileWithUniqueCrackedPass))
    else:
        print('--> 0/%i unique passwords were cracked.' %
              len(finalUniqueUsersRecords))
...
def crackRepeatedPasswords():
    ...
    if os.path.exists(outputFileWithRepeatedCrackedPass):
        with open(outputFileWithRepeatedCrackedPass, 'r') as file:
            passwordsCracked = len(file.readlines())
            if passwordsCracked > 0:
                print('--> A total of %i repeated passwords have been cracked successfully. Check the results in "%s."' %
                      (len(file.readlines()), outputFileWithRepeatedCrackedPass))
            else:
...
def crackUniquePasswords():
    ...
    if os.path.exists(outputFileWithUniqueCrackedPass):
        with open(outputFileWithUniqueCrackedPass, 'r') as file:
            passwordsCracked = len(file.readlines())
            if passwordsCracked > 0:
                print("--> A total of %i passwords have been cracked successfully. Check the results in %s." %
                      (len(file.readlines()), outputFileWithUniqueCrackedPass))
            else:
...
def crackPasswordsManager():
    if len(userRepeatedPass) > 0:
        ...
        if input('If you want to continue please, press Enter. ') == "":
            ...
            print('\nThere is a total of %i unique passwords in "%s". Please be aware this process can take a long time to complete.' % (
                len(finalUniqueUsersRecords), fileWithUniquePass))
            if input('If you want to continue please, press Enter. ') == "":
...
with open(usersPathFile, newline='') as csvfile:
    ...
    for col in file:
        ...
        if (len(col['user_password']) > 0 and col['user_password'] != '<blank>' and col['user_password'] != ''):
            finalUniqueUsersRecords.append(
                {'password': col['user_password'], 'username': col['user_name']})

    if (len(finalUniqueUsersRecords) > 0):
        print('**** Initial file stats:')
        print(f'Total of records in file: {str(totalFileRecords)}.')
        print(
            f'Users with empty password: {str(totalFileRecords - len(finalUniqueUsersRecords))}.')
        print(
            f'--> Final number of records to analise: {str(len(finalUniqueUsersRecords))}.')
        ...
        if (getUsersWithSamePassword()):
            ...
            if hashcatChecker[0] < 0:
                print('\nThe file "%s" has been analised succesfully.' %
                      os.path.basename(usersPathFile))
